...
function usuariosA( $atts ){
	global $wpdb;
	
	$resultado = $wpdb->get_results ("SELECT * FROM usuariosa");
	...
	foreach($resultado as $prd){
		...
		$html.= '<td>'.$prd->nome.'</td>';
		$html.= '<td>'.$prd->email.'</td>';
		$html.= '<td>'.$prd->nascimento.'</td>';
		$html.= '<td>'.$prd->senha.'</td>';
		$html.= '<td>'.$prd->comentarios.'</td>';
		$html.= '<td>'.$prd->valor_carteira.'</td>';
		$html.= '<td>'.$prd->tipo.'</td>';
...
add_shortcode( 'usuarios', 'usuariosA');

function usuariosB( $atts ){
	global $wpdb;
	
	$resultado = $wpdb->get_results ("SELECT * FROM usuariob");
...
add_shortcode( 'usuarios1', 'usuariosB');